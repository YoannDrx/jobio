...
} from "./prompts/linkedin-audit.prompt";

const linkedinAuditInputSchema = z.object({
...
function buildPrompt(profile: {
  ...
  let prompt = `## Profil LinkedIn a auditer\n`;
  prompt += `- Nom : ${profile.name}\n`;
  prompt += `- Headline : ${profile.headline}\n`;
  if (profile.bio) prompt += `- Bio/About : ${profile.bio}\n`;
  ...
  prompt += `\nRealise un audit complet de ce profil LinkedIn. Evalue chaque section et propose un plan d'action sur 30 jours.`;
  return prompt;
}
