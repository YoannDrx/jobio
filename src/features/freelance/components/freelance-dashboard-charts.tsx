...
export function FreelanceDashboardCharts({ charts }: FreelanceDashboardChartsProps) {
  if (!charts) {
    return null;
  }
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={charts.revenueTimeline} margin={{ left: 0, right: 16, top: 8 }}>
      ...
      <Card>
        ...
        <CardContent>
          {statusData.length === 0 ? (
            <p className="text-muted-foreground text-sm">Pas encore de données facture.</p>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={charts.topClients} margin={{ left: 0, right: 16, top: 8 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="clientLabel" tick={{ fontSize: 11 }} interval={0} angle={-14} textAnchor="end" height={60} />
              <YAxis tickFormatter={formatCompactCents} width={80} />
              <Tooltip formatter={(value: number) => formatCents(value)} />
              <Legend />
              <Bar dataKey="invoicedCents" fill="#06b6d4" name="Facturé" radius={[6, 6, 0, 0]} />
              <Bar dataKey="outstandingCents" fill="#fb923c" name="Encours" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {charts.declarationBreakdown.length === 0 ? (
            <p className="text-muted-foreground text-sm">Aucune période déclarative en cours.</p>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={charts.declarationBreakdown} margin={{ left: 0, right: 16, top: 8 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="periodKey" />
                <YAxis tickFormatter={formatCompactCents} width={80} />
                <Tooltip formatter={(value: number) => formatCents(value)} />
                <Legend />
                <Bar dataKey="invoicedCents" fill="#22c55e" name="Facturé" radius={[6, 6, 0, 0]} />
                <Bar dataKey="socialChargesCents" fill="#f59e0b" name="Charges estimées" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </CardContent>
