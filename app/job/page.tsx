...
export default async function TodayPage() {
  ...
  const [
    ...
    // Missions with at least one follow-up this week
    prisma.mission.count({
      where: {
        userId: user.id,
        createdAt: {
    ...
    // Total missions created this week
    prisma.mission.count({
      where: {
        userId: user.id,
        createdAt: {
  ...
  let weekResponseRate: number | null = null;
  if (weekTotalMissions >= 5) {
    weekResponseRate = (weekMissionsWithResponse / weekTotalMissions) * 100;
  }
