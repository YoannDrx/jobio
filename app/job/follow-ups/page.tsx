...
import { AIMessageSheet } from "@/features/follow-ups/components/ai-message-sheet";
import {
  List,
  CalendarDays,
  Plus,
...
function getInDays(days: number): string {
  const date = new Date();
...
export default function FollowUpsPage() {
  ...
  const [aiMessageSheetOpen, setAiMessageSheetOpen] = useState(false);
  const [aiMessageFollowUp, setAiMessageFollowUp] =
    useState<FollowUpForCalendar | null>(null);
  const [createPrefilledDate, setCreatePrefilledDate] = useState<Date | null>(
    null,
  );
    ...
    <Layout size="lg">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {!fu.completedAt && (
                                <DropdownMenuItem
                                  onClick={() => {
                                    setAiMessageFollowUp(fu);
                                    setAiMessageSheetOpen(true);
                                  }}
                                >
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-4">
            ...
            <FollowUpForm
              defaultValues={
                createPrefilledDate
                  ? { scheduledAt: createPrefilledDate }
                  : undefined
              }
      ...
      >
        <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
          <SheetHeader>
            <SheetTitle>Modifier la relance</SheetTitle>
            {editingFollowUp?.mission && (
              <p className="text-muted-foreground text-sm">
                Mission : {editingFollowUp.mission.title}
                {editingFollowUp.mission.company &&
                  ` — ${editingFollowUp.mission.company}`}
              </p>
            )}
          </SheetHeader>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSnoozeDate(getTomorrow())}
                >
                  Demain
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSnoozeDate(getInDays(3))}
                >
                  +3 jours
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSnoozeDate(getInDays(7))}
                >
                  +7 jours
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setSnoozeDate(getInDays(14))}
                >
                  +14 jours
                </Button>
              </div>
      ...
      {aiMessageFollowUp && (
        <AIMessageSheet
          isOpen={aiMessageSheetOpen}
          onOpenChange={setAiMessageSheetOpen}
          missionId={aiMessageFollowUp.missionId}
          followUpType={aiMessageFollowUp.type}
        />
      )}
    </Layout>
  );
