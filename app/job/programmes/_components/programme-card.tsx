...
const PROGRAM_STYLES: Record<
  string,
  { border: string; badge: string; glow: string; cta: string }
> = {
  "se-lancer-linkedin": {
      ...
      <CardFooter>
        {program.isUnlocked ? (
          <Button asChild variant="outline" className="w-full">
            <Link href={`/job/programmes/${program.slug}`}>
              Voir les templates
            </Link>
          </Button>
        ) : (
          <Button asChild className={`w-full truncate ${style.cta}`}>
            <Link href={`/job/programmes/${program.slug}`}>
              <Lock className="size-4 shrink-0" />
              <span className="truncate">
                {program.isFree
                  ? "Débloquer gratuitement"
                  : `Débloquer – ${(program.price / 100).toFixed(0)} €`}
              </span>
            </Link>
          </Button>
        )}
