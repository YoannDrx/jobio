...
test.describe("freelance-billing", () => {
  // Skip test in CI - requires complex setup that needs investigation
  test(
    "quote to invoice payment flow",
    ...
    async ({ page }) => {
      ...
      try {
        ...
        await page
          ...
          .click();
        await expect(
          page.getByText("Devis brouillon créé").first(),
        ).toBeVisible({
          timeout: 10000,
        });
        ...
        await expect(
          page
            .getByRole("heading", { name: "Enregistrer un paiement" })
            .first(),
        ).toBeVisible({ timeout: 10000 });
