...
test.describe("cv-lab-coach", () => {
  ...
  test(
    "cv coach creates session and sends message",
    { skip: isCI },
    async ({ page }) => {
  ...
  test(
    "cv coach imports text and triggers analysis",
    { skip: isCI },
    async ({ page }) => {
      const userData = await createTestAccount({
        page,
      ...
      try {
        ...
        await page
          .getByRole("button", { name: "Importer et analyser" })
          .click();
        ...
        expect(
          (sessionAfter as typeof sessionAfter & { messages: unknown[] })
            .messages.length,
        ).toBeGreaterThan(0);
      } finally {
        const user = await prisma.user.findUnique({
